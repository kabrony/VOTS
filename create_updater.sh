cat << 'EOF' > update_nextjs_docker.sh
#!/usr/bin/env bash
set -e
...
chmod +x update_nextjs_docker.sh

echo "Script 'update_nextjs_docker.sh' created. Now run it via:"
echo "  bash update_nextjs_docker.sh"
echo "It will overwrite Dockerfile + .dockerignore in 'nextjs_dashboard/'."
