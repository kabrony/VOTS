#!/usr/bin/env python3
"""
v01.06_autonomous_all_in_one.py
==========================================================
Autonomous microservice manager for ORGANIX.ETH, with
AI-driven improvements (GPT-4 + Gemini synergy), Docker
health checks, code refactoring, finance checks, and
Socket.IO server on a flexible port to avoid "address in use."

Usage:
  1) python v01.06_autonomous_all_in_one.py
  2) If port 5000 is busy, it automatically tries 5050
  3) .env controls environment variables (OPENAI_API_KEY, etc.)

Prerequisites:
  - You installed the flexible 'requirements.txt'
  - Docker Compose + Docker installed
  - patch installed (for code refactor diffs)
...
\033[35m ██████     ██     ██████  ██      ██  ██████  ██   ████ ██ 
\033[35m
 ORGANIX.ETH :: AUTONOMOUS SWARM v01.06
\033[32m
        ...
        for line in lines:
            ...
            if any(x in cstat_lower for x in ["unhealthy","restarting","dead"]):
                ...
                if GEMINI_FLASH_ENABLED:
                    gemini_flash_intelligence(f"Container {cont_name} is {cstatus}, fix needed.")
    except subprocess.CalledProcessError as e:
...
def gemini_flash_intelligence(context_str):
    ...
    """
    log_info(f"[GeminiFlash] synergy => {context_str}")
...
"""
    ...
    if combined.strip():
        ...
        log_info("[GeminiFlash] synergy => no actionable content from AI.")
...
def apply_patch(base_dir, patch_text):
    ...
    try:
        ...
        for pnum in ["1","2"]:
            ...
            log_warn("[Refactor] Patch failed with both -p1 and -p2. Review manually.")
    finally:
...
def manage_finances():
    ...
    try:
        ...
        if AUTONOMOUS_FINANCE_ANALYSIS_TRIGGER and bal_eth < ETH_BALANCE_THRESHOLD_FOR_AI:
            gemini_flash_intelligence(f"Low ETH balance => {bal_eth}, advise actions?")
    except Exception as e:
...
def find_free_port(preferred=5000, fallback=5050):
    """
    Check if `preferred` is free, else return `fallback`.
    """
...
def run_socketio_server(port):
    ...
    try:
        ...
        log_info(f"Socket.IO => listening on port {port}")
...
def main():
    print(ASCII_BANNER)
    log_info("Starting v01.06_autonomous_all_in_one for organix.eth ...")

    # Start orchestrator in background
    t = threading.Thread(target=orchestrator_loop, daemon=True)
    t.start()

    # Attempt to listen on 5000, fallback if busy
